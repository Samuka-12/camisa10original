...
const ProductCard = ({ id, image, name, team, price, priceNum, category, oldPrice, priority = false }: ProductCardProps) => {
  ...
  const productUrl = id ? `/produto/${id}` : "#";

  return (
    <Link to={productUrl} className="block group">
      <div className="bg-card rounded-xl border border-border overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
        ...
        <div className="p-4 flex flex-col flex-1 justify-between">
          ...
          {/* ── Botão Comprar (Leva para o conteúdo do produto) ── */}
          <div
            style={pulse?.ativo ? {
            ...
            className={`mt-4 w-full py-2.5 rounded-lg bg-accent text-accent-foreground text-sm font-semibold text-center group-hover:bg-accent/90 transition-colors ${pulseClass}`}
          >
            Comprar
          </div>
        </div>
